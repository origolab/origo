...
/// The Sapling Incoming viewing key(ivk in protocol)
pub type IncomingViewingKey<E: JubjubEngine> = E::Fs;

pub struct ViewingKey<E: JubjubEngine> {
    pub ak: edwards::Point<E, PrimeOrder>,
    pub nk: edwards::Point<E, PrimeOrder>
}
impl<E: JubjubEngine> PartialEq for ViewingKey<E> {
    fn eq(&self, other: &ViewingKey<E>) -> bool {
        self.ak == other.ak && self.nk == other.nk
    }
}
impl<E: JubjubEngine> Eq for ViewingKey<E> {}
impl<E: JubjubEngine> Hash for ViewingKey<E> {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.ak.hash(state);
        self.nk.hash(state);
    }
}
impl<E: JubjubEngine> Copy for ViewingKey<E> {}
impl<E: JubjubEngine> Clone for ViewingKey<E> {
    fn clone(&self) -> Self {
        ViewingKey {
            ak: self.ak.clone(),
            nk: self.nk.clone(),
...
/// The Sapling Diversifier(d in protocol)
...
impl<E: JubjubEngine> Hash for PaymentAddress<E> {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.diversifier.0.hash(state);
        self.pk_d.hash(state);
    }
}
impl<E: JubjubEngine> Copy for PaymentAddress<E> {}
impl<E: JubjubEngine> PartialEq for PaymentAddress<E> {
    fn eq(&self, other: &PaymentAddress<E>) -> bool {
        self.pk_d == other.pk_d && self.diversifier.0 == other.diversifier.0
    }
}
...
/// A note represents that a value v is spendable by the recipient who holds
/// the spending key corresponding to a given shielded payment address.
#[derive(Clone, Debug)]
pub struct Note<E: JubjubEngine> {
    /// The value of the note in zatoshi
    pub value: u64,
    /// the diversifier of the recipient’s shielded payment address; GH(d)
    pub g_d: edwards::Point<E, PrimeOrder>,
    /// the diversified transmission key of the recipient’s shielded
    /// payment address; g_d^ivk
    pub pk_d: edwards::Point<E, PrimeOrder>,
