...
impl rlp::Decodable for Signature {
    fn decode(d: &Rlp) -> Result<Self, DecoderError> {
        ...
        let mut rbar: [u8;32] = [0;32];
        let rbar_vec: Vec<u8> = d.val_at(0)?;
        let mut sbar: [u8;32] = [0;32];
        let sbar_vec: Vec<u8> = d.val_at(1)?;
        if rbar_vec.len() != rbar.len() || sbar_vec.len() != sbar.len() {
            return Err(DecoderError::Custom("Invalid Signature."));
        }
        rbar.copy_from_slice(&rbar_vec);
        sbar.copy_from_slice(&sbar_vec);
...
impl<E: JubjubEngine> Eq for PublicKey<E> {}
...
mod tests {
    ...
    fn encode_decode_signature() {
        ...
        let encoded = ::rlp::encode(&signature1);
	    let signature2 : Signature = rlp::decode(&encoded).unwrap();
