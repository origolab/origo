...
// TODO(xin): Move key struct from zip32 to here.

/// The Sampling Key Structure
///                         ---- Payment Address----
///                        /                        \
...
///                                            Spending Key
///
/// The spending authorizing key ask, proof authorizing key (ak, nsk),
/// full viewing key (ak, nk, ovk), incoming viewing key ivk and
/// each diversified payment address(d, PKd) are derived from sk.
...
impl<E: JubjubEngine> Copy for ExpandedSpendingKey<E> {}
...
impl<E: JubjubEngine> PartialEq for FullViewingKey<E> {
    fn eq(&self, other: &FullViewingKey<E>) -> bool {
        self.vk == other.vk && self.ovk == other.ovk
    }
}
impl<E: JubjubEngine> Eq for FullViewingKey<E> {}
impl<E: JubjubEngine> Hash for FullViewingKey<E> {
...
impl<E: JubjubEngine> Copy for FullViewingKey<E> {}
