...
pub fn load_single_parameters(
    path: &Path,
    hash: &str) ->  (Parameters<Bls12>, PreparedVerifyingKey<Bls12>){
    let fs = File::open(path).expect("couldn't load Sapling parameters file");
    let mut fs = hashreader::HashReader::new(BufReader::with_capacity(1024 * 1024, fs));
    let params = Parameters::<Bls12>::read(&mut fs, false)
        .expect("couldn't deserialize parameters file");
    let mut sink = io::sink();
    io::copy(&mut fs, &mut sink)
        .expect("couldn't finish reading Sapling parameter file");
    if fs.into_hash() != hash {
        panic!("Sapling parameter file is not correct, please clean your `~/.zcash-params/` and re-run `fetch-params`.");
    }
    let vk = prepare_verifying_key(&params.vk);
    (params, vk)
}
pub fn load_parameters_from_bytes(bytes: &[u8])
    -> (Parameters<Bls12>, PreparedVerifyingKey<Bls12>){
    let params = Parameters::<Bls12>::read(bytes, false)
        .expect("couldn't deserialize parameters file");
    let vk = prepare_verifying_key(&params.vk);
    (params, vk)
}
