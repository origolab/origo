...
use blockchain::wallet::wallet::Wallet;
use blockchain::{
...
use types::receipt::{LocalizedReceipt, Receipt};
use types::transaction::{
	self, Action, LocalizedTransaction, SignedTransaction, UnverifiedTransaction,
};
use types::{
...
use client::{
	AccountData, Balance, BlockChain as BlockChainTrait, BlockChainReset, BlockInfo, BlockProducer,
	BroadcastProposalBlock, Call, ChainInfo, ClientIoMessage, ImportBlock, ImportSealedBlock,
	Nonce, PrepareOpenBlock, ReopenBlock, ScheduleInfo, SealedBlockImporter, StateClient,
	StateInfo, StateOrBlock, TransactionInfo,
};
use client::{
	BadBlocks, BlockChainClient, BlockId, CallAnalytics, ChainMessageType, ChainNotify, ChainRoute,
	ClientConfig, EngineInfo, IoClient, Mode, NewBlocks, ProvingBlockChainClient, PruningInfo,
	TraceFilter, TraceId, TransactionId, UncleId,
};
use engines::epoch::PendingTransition;
use engines::{EngineError, EpochTransition, EthEngine, ForkChoice, MAX_UNCLE_AGE};
use error::{
	BlockError, CallError, Error as EthcoreError, ErrorKind as EthcoreErrorKind, EthcoreResult,
	ExecutionError, ImportErrorKind, QueueError, QueueErrorKind,
};
...
use types::transaction::SyncTransaction;
use verification;
use verification::queue::kind::blocks::Unverified;
use verification::queue::kind::BlockLike;
use verification::{BlockQueue, PreverifiedBlock, Verifier};
...
impl Importer {
	...
	fn import_old_block(
		...
		//println!("In client import_old_block");
	...
	where
		B: Drain,
	{
		//println!("In client commit_block");
		let hash = &header.hash();
		...
		client.update_engine_rewards(number, fork_choice, &route);
...
impl Client {
	...
	pub fn new(
		...
		// store era reward into engine.
		spec.engine.update_rewards(client.chain.read().get_rewards());
	...
	fn update_engine_rewards(&self, number: BlockNumber, fork_choice: ForkChoice, route: &ImportRoute) {
		trace!(target: "client", "update_engine_rewards block number {:?}", number);

		match  fork_choice {
			ForkChoice::New => {
				if route.retracted.len() > 0 {
					// insert block on BranchBecomingCanonChain, need update engine rewards.
					trace!(target: "client", "update rewards on BranchBecomingCanonChain");
					self.engine.update_rewards(self.get_rewards());
				} else {
					// insert block on CanonChain, check is last block of era.
					if self.config.blockchain.reward_config.is_era_last_block(number) {
						trace!(target: "client", "update rewards on new era");
						self.engine.update_rewards(self.get_rewards());
					}
				}
			},
			ForkChoice::Old => {
				// insert block on Branch, don't need to update engine rewards.
			},
		}
	}
	...
	pub fn wallet(&self) -> Arc<STD_RwLock<Wallet>> {
		self.chain.read().pt_wallet()
	}
...
impl BlockChainReset for Client {
	fn reset(&self, num: u32) -> Result<(), String> {
		...
		let (blocks_to_delete, best_block_hash) = self
			.chain
			.read()
		...
		// update rewards.
		let mut rewards = self.chain.read().get_rewards();
		let hashs: Vec<H256> = blocks_to_delete.iter().map(|h|h.hash()).collect();

		// reverse rewards to the reset block.
		if self.chain.read().do_rewards_of_batch_blocks(&mut rewards, &hashs, true) {
			// update rewards.
			db_transaction.put(db::COL_EXTRA, b"Rewards", &rlp::encode(&rewards));
		}
		...
		self.db
			...
			.map_err(|err| {
				format!(
					"could not complete reset operation; io error occured: {}",
					err
				)
			})?;
...
impl BlockInfo for Client {
	...
	fn commitment_root_block(&self, root: &Node) -> Option<H256> {
		self.chain.read().commitment_root_block(root)
	}
}
...
impl TransactionInfo for Client {
	...
	fn nullifier_transaction(&self, nullifier: &U256) -> Option<H256> {
		self.chain.read().nullifier_transaction(&nullifier)
	}
}
...
impl BlockChainClient for Client {
	...
	fn get_rewards(&self) -> Rewards { self.chain.read().get_rewards() }

	fn get_total_reward(&self) -> (BlockNumber, U256) { self.chain.read().get_total_reward() }

	#[cfg(test)]
	fn update_rewards(&self, eras: Rewards) { self.chain.read().update_rewards(eras) }
...
/*pub fn debug_println( block: &SealedBlock){
	for tx in block.transactions.iter() {
		println!("transaction, v_shield_outputs.len = {0}",
				 tx.transaction.unsigned.v_shielded_output().len());
	}
}*/

/*pub fn debug_println_exe( block: &ExecutedBlock){
	for tx in block.transactions.iter() {
		println!("transaction, v_shield_outputs.len = {0}",
				 tx.transaction.unsigned.v_shielded_output().len());
	}
...
impl ImportSealedBlock for Client {
	fn import_sealed_block(&self, block: SealedBlock) -> EthcoreResult<H256> {
		//println!("In import_sealed_block");
		//debug_println(&block);
		let start = Instant::now();
...
impl super::traits::EngineClient for Client {
	...
	fn submit_seal(&self, pow_hash: H512, seal: Vec<Bytes>) {
		let import = self
			.importer
			.miner
...
fn transaction_receipt(
	...
	LocalizedReceipt {
		from: sender,
		to: match tx.action {
			Action::Create | Action::PrivateToPrivate | Action::PublicToPrivate => None,
			Action::Call(ref address) | Action::PrivateCall(ref address) =>
		...
		contract_address: match tx.action {
			Action::Call(_) | Action::PrivateToPrivate | Action::PublicToPrivate | Action::PrivateCall(_) => None,
			Action::Create => Some(
				contract_address(
...
mod tests {
	...
	fn should_return_correct_log_index() {
		...
		use types::receipt::{LocalizedReceipt, Receipt, TransactionOutcome};
		use types::transaction::{Action, LocalizedTransaction, Transaction};
