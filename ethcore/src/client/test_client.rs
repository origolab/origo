...
use types::transaction::{
	self, Action, LocalizedTransaction, SignedTransaction, Transaction, UnverifiedTransaction,
};
use types::view;
...
impl BlockInfo for TestBlockChainClient {
	...
	/// TODO(Kui): In test case, we assume the anchor exist in block for mocked private transaction.
	/// Indeed we should verify the anchor for valid private transaction later.
...
impl BlockChainClient for TestBlockChainClient {
	...
	fn get_rewards(&self) -> Rewards {Rewards::new() }

	fn get_total_reward(&self) -> (BlockNumber, U256) { (0, U256::zero()) }
