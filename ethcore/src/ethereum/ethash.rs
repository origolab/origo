...
use types::rewards::{EraRewardInfo, Rewards};
use unexpected::{OutOfBounds, Mismatch};

use block::ExecutedBlock;
use engines::block_reward::{self, BlockRewardContract, RewardKind};
use engines::{self, Engine, EngineError};
use error::{BlockError, Error};
use machine::EthereumMachine;
use types::solution::{EquihashSolution};
use verification::equihash_verifier::{verify_block_solution, verify_block_pow};
...
impl Seal {
	...
	pub fn parse_seal<T: AsRef<[u8]>>(seal: &[T]) -> Result<Self, Error> {
		...
		let nonce = Rlp::new(seal[0].as_ref()).as_val::<H256>()?;
		let solution = Rlp::new(seal[1].as_ref()).as_val::<EquihashSolution>()?;
...
impl From<ethjson::spec::EthashParams> for EthashParams {
	fn from(p: ethjson::spec::EthashParams) -> Self {
		EthashParams {
			...
			eip100b_transition: p.eip100b_transition.map_or(0, Into::into),
...
impl Engine<EthereumMachine> for Arc<Ethash> {
	...
	fn extra_info(&self, header: &Header) -> BTreeMap<String, String> {
		match Seal::parse_seal(header.seal()) {
			Ok(seal) => map![
				...
				"solution".to_owned() => format!("0x{:x}", seal.solution)
			],
			_ => BTreeMap::default()
		}
	}

	fn maximum_uncle_count(&self, _block: BlockNumber) -> usize { self.machine.params().reward_config.maximum_uncle_count as usize }
	...
	fn on_close_block(&self, block: &mut ExecutedBlock) -> Result<(), Error> {
		...
		let rewards = match self.ethash_params.block_reward_contract {
			...
			_ => {
				...
				let  base_reward = match  self.ethash_params.block_reward.iter()
					.rev().find(|&(block, _)| *block <= number){
					Some(r)=> *r.1,
					None => {
						let era = self.machine.params().reward_config.calc_era_by_block_number(number);
						match self.rewards.read().get_era_base_reward(era as usize) {
							Some(r) => r,
							None => U256::zero(),
						}
					}
				...
				trace!(target: "equihash","number {:?} base reward {:?}", number, base_reward);

				// Applies ECIP-1017 eras.
				let eras_rounds = self.ethash_params.ecip1017_era_rounds;
				let (eras, reward) = ecip1017_eras_block_reward(eras_rounds, base_reward, number);

				//let n_uncles = LiveBlock::uncles(&*block).len();
				let n_uncles = block.uncles.len();

				// Bestow block rewards.
				let author_reward = self.machine.params().reward_config.calc_author_miner_reward(reward, n_uncles);
				trace!(target: "equihash","result_block_reward {:?}", author_reward);
				...
				for u in &block.uncles {
					let uncle_author = u.author();
					let mut uncle_reward = if eras == 0 {
						self.machine.params().reward_config.calc_uncle_miner_reward(reward, number, u.number())
					} else {
						reward.shr(5)
					};

					trace!(target: "equihash","result_uncle_reward {:?}", uncle_reward);
					rewards.push((*uncle_author, RewardKind::uncle(number, u.number()), uncle_reward));
				}
	...
	/// update era rewards.
	fn update_rewards(&self, eras: Rewards){
		let mut rewards = self.rewards.write();
		*rewards = eras;
	}

	#[cfg(test)]
	fn get_rewards(&self) -> Option<Rewards> { Some(self.rewards.read().clone()) }
}
...
impl Ethash {
	fn calculate_difficulty(&self, header: &Header, parent: &Header) -> U256 {
		...
		let mut target = if header.number() < frontier_limit {
			...
			trace!(target: "equihash", "Calculating difficulty block {:?} parent.difficulty={}, header.timestamp={}, parent.timestamp={}", header.number(), parent.difficulty(), header.timestamp(), parent.timestamp());
...
mod tests {
	...
	use rlp;
	use tempdir::TempDir;
	use types::solution::{EquihashSolution, SOLUTION_SIZE};
	use types::rewards::{Rewards, EraRewardInfo};
	...
	fn test_spec() -> Spec {
		...
		let base_reward = config.reward_config.calc_era_base_reward(U256::zero());
		let mut era_rewards = Rewards::new();
		era_rewards.add_new_era_reward(EraRewardInfo::new(0,U256::zero(), base_reward));

		spec.engine.update_rewards(era_rewards);
		spec
	}
	...
	fn on_close_block() {
		...
		assert_eq!(b.state.balance(&Address::zero()).unwrap(), U256::from_str("128ff8f22963b1200").unwrap());
	}
	...
	fn on_close_block_with_uncle() {
		...
		let b = b.close().unwrap();
		assert_eq!(b.state.balance(&Address::zero()).unwrap(), "132478b9b83cf5600".into());
		assert_eq!(b.state.balance(&uncle_author).unwrap(), "103df9d3e2d19a400".into());
	}
	...
	fn can_do_seal_verification_fail() {
		...
		match verify_result {
			...
			Err(_) => { panic!("should be block seal invalid  error (got {:?})", verify_result); },
	...
	fn can_do_proof_of_work_verification_fail() {
		...
		let s:String = solution.into_iter().collect();
		header.set_seal(vec![rlp::encode(&H256::zero()), rlp::encode(&s)]);
		header.set_difficulty(U256::from_str("ffffffffffffffffffffffffffffffffffffffffffffaaaaaaaaaaaaaaaaaaaa").unwrap());
		...
		match verify_result {
			...
			Err(_) => { panic!("should be invalid solution error (got {:?})", verify_result); },
			_ => { panic!("Should be error, got Ok"); },
		}
	}

	#[test]
	fn can_do_seal_basic_verification_fail2() {
		let engine = test_spec().engine;
	...
	#[test]
	fn can_do_block_unordered_verification_fail() {
		let engine = test_spec().engine;
		let mut header: Header = Header::default();
		let solution:[char;SOLUTION_SIZE] = ['0'; SOLUTION_SIZE];
		let s:String = solution.into_iter().collect();
		header.set_seal(vec![rlp::encode(&H256::from("b251bd2e0283d0658f2cadfdc8ca619b5de94eca5742725e2e757dd13ed7503d")), rlp::encode(&s)]);
		header.set_difficulty(U256::from_str("ffffffffffffffffffffffffffffffffffffffffffffaaaaaaaaaaaaaaaaaaaa").unwrap());

		let verify_result = engine.verify_block_unordered(&header);

		match verify_result {
			Err(Error(ErrorKind::Block(BlockError::InvalidSolution), _)) => {},
	...
	fn difficulty_max_timestamp() {
		...
		let difficulty = ethash.calculate_difficulty(&header, &parent_header);
		assert_eq!(U256::from(12543204905463u64), difficulty);
	}
	...
	fn test_extra_info() {
		...
		let ethash = Ethash::new(ethparams, machine);
		let mut header = Header::default();
		let solution = format!("{:0>800}", "0");
		let vec_solution = vec![0;400];
		header.set_seal(vec![rlp::encode(&H256::from("b251bd2e0283d0658f2cadfdc8ca619b5de94eca5742725e2e757dd13ed7503d")), rlp::encode(&vec_solution)]);
		let info = ethash.extra_info(&header);
		assert_eq!(info["nonce"], "0xb251bd2e0283d0658f2cadfdc8ca619b5de94eca5742725e2e757dd13ed7503d");
		assert_eq!(info["solution"], "0x".to_string() + solution.as_ref());
	}
}
