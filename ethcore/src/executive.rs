...
use std::sync::Arc;
use hash::keccak;
use ethereum_types::{H256, U256, U512, H160, Address};
...
use types::transaction::{Action, SignedTransaction, PRIVATE_EXECUTIVE_ADDRESS};
...
impl<'a> CallCreateExecutive<'a> {
	...
	fn transfer_exec_balance<B: 'a + StateBackend>(params: &ActionParams, schedule: &Schedule, state: &mut State<B>, substate: &mut Substate) -> vm::Result<()> {
		if let ActionValue::Transfer(val) = params.value {
			match (params.is_input_private, params.is_output_private) {
				(false, false) => state.transfer_balance(&params.sender, &params.address, &val, substate.to_cleanup_mode(&schedule))?,
				(true, false) => state.transfer_balance_from_private_to_public(&params.sender, &params.address, &val, substate.to_cleanup_mode(&schedule))?,
				(false, true) => state.transfer_balance_from_public_to_private(&params.sender, &params.address, &val, substate.to_cleanup_mode(&schedule))?,
				(true, true) => (),
			}
		}
...
impl<'a, B: 'a + StateBackend> Executive<'a, B> {
	...
	fn transact_with_tracer<T, V>(
		...
		if (balance512 < total_cost) & !t.action.is_input_private() {
		...
		match t.action {
			Action::PrivateToPrivate => {
				let params = ActionParams {
					code_address: PRIVATE_EXECUTIVE_ADDRESS,
					address: PRIVATE_EXECUTIVE_ADDRESS,
					sender: PRIVATE_EXECUTIVE_ADDRESS,
					origin: PRIVATE_EXECUTIVE_ADDRESS,
					gas: init_gas,
					gas_price: t.gas_price,
					value: ActionValue::Transfer(t.value),
					code: self.state.code(&PRIVATE_EXECUTIVE_ADDRESS)?,
					code_hash: self.state.code_hash(&PRIVATE_EXECUTIVE_ADDRESS)?,
					data: Some(t.data.clone()),
					call_type: CallType::Call,
					params_type: vm::ParamsType::Separate,
					is_input_private: true,
					is_output_private: true,
				};
				...
				// finalize here!
				Ok(self.finalize(t, substate, res, out, tracer.drain(), vm_tracer.drain())?)
			}
			...
			Action::PrivateCall(ref address) => {
				let params = ActionParams {
	...
	fn finalize<T, V>(
		...
		if t.transaction.action.is_input_private() {
			self.state.add_balance(&PRIVATE_EXECUTIVE_ADDRESS, &refund_value, CleanupMode::NoEmpty)?;
		} else {
...
mod tests {
	...
	// test is incorrect, mk
	// TODO: fix (preferred) or remove
	evm_test!{test_transact_simple: test_transact_simple_int}
	fn test_transact_simple(factory: Factory) {
		...
		println!("executed.gas_used:{}, executed.refunded:{}", executed.gas_used, executed.refunded);
	...
	evm_test!{test_transact_simple_public_to_private: test_transact_simple_public_to_private_int}
	fn test_transact_simple_public_to_private(factory: Factory) {
		let keypair = Random.generate().unwrap();

		let t = Transaction::create_executive_transaction(vec![0], U256::zero(), Action::PublicToPrivate,
														  U256::from(50_000), U256::from(100_000), U256::from(1)).sign(keypair.secret(), None);

		let sender = t.sender();
		let contract = contract_address(CreateContractAddress::FromSenderAndNonce, &sender, &U256::zero(), &[]).0;

		let mut state = get_temp_state_with_factory(factory);
		state.add_balance(&sender, &U256::from(200_000), CleanupMode::NoEmpty).unwrap();
		assert_eq!(state.balance(&sender).unwrap(), U256::from(200_000));
		let mut info = EnvInfo::default();
		info.gas_limit = U256::from(100_000);
		let machine = make_frontier_machine(0);
		let schedule = machine.schedule(info.number);
		...
		assert_eq!(executed.cumulative_gas_used, U256::from(21000));
		assert_eq!(executed.logs.len(), 0);
		assert_eq!(executed.contracts_created.len(), 0);
		assert_eq!(state.balance(&sender).unwrap(), U256::from(50000 + 100_000 - 21000));
		assert_eq!(state.balance(&contract).unwrap(), U256::from(0));
		assert_eq!(state.nonce(&sender).unwrap(), U256::from(1));
	}

	evm_test!{test_transact_simple_private_call: test_transact_simple_private_call_int}
	fn test_transact_simple_private_call(factory: Factory) {
		let receiver = Address::from_str("cd1722f3947def4cf144679da39c4c32bdc35681").unwrap();
		let t = Transaction::create_executive_transaction(vec![0], U256::zero(), Action::PrivateCall(receiver),
														  U256::from(100_000), U256::from(100_000), U256::from(1)).sign_for_private(0);
		let sender = t.sender();
		...
		assert_eq!(state.balance(&receiver).unwrap(), U256::from(100_000));
		assert_eq!(state.nonce(&sender).unwrap(), U256::from(0));
		assert_eq!(state.balance(&PRIVATE_EXECUTIVE_ADDRESS).unwrap(), U256::from(100_000 - 21000));
	}

	evm_test!{test_transact_simple_private_to_private: test_transact_simple_private_to_private_int}
	fn test_transact_simple_private_to_private(factory: Factory) {
		let t = Transaction::create_executive_transaction(vec![0], U256::zero(), Action::PrivateToPrivate,
														  U256::zero(), U256::from(100_000), U256::from(1)).sign_for_private(0);
		let sender = t.sender();
