...
pub fn json_chain_test<H: FnMut(&str, HookType)>(json_data: &[u8], start_stop_hook: &mut H) -> Vec<String> {
	...
	for (name, blockchain) in tests.into_iter() {
		...
		let mut fail = false;
		{
			let mut fail_unless = |client:H256 , chain, last_block:H256| if client != chain && !fail {
				println!("json_chain_test check error  name {:?} client: {:?} chain: {:?} last_block {:?} ", name ,client, chain, last_block);
...

			{
				...
				//TODO(eric) need add json file to do equihash pow test
				...
				for b in blockchain.blocks_rlp() {
					if let Ok(block) = Unverified::from_rlp(b) {
						last_block_hash = H256::from(block.hash().0);
						let parent_hash = H256::from(block.parent_hash().0);
						if let Err(e) = client.import_block(block){
							println!("error importing block which is valid by definition: {:?} genesis {:?} latest {:?}  insert block parent {:?} insert block hash {:?} ",
							         e,spec.genesis_header().hash(),client.chain_info().best_block_hash, parent_hash, last_block_hash);
						}
						client.flush_queue();
				...
				fail_unless(client.chain_info().best_block_hash, blockchain.best_block.into(), last_block_hash);
			}
		}
