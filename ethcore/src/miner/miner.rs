...
impl Miner {
	...
	pub fn new<A: LocalAccounts + 'static>(
		...
		Miner {
			...
			options,
			transaction_queue: Arc::new(TransactionQueue::new(limits, verifier_options, tx_queue_strategy, sender)),
	...
	pub fn new_for_tests(spec: &Spec, accounts: Option<HashSet<Address>>) -> Miner {
		...
		Miner::new(MinerOptions {
			...
			..Default::default()
		}, GasPricer::new_fixed(minimal_gas_price), spec, accounts.unwrap_or_default(), None)
	}
		...
		C: BlockChain + CallContract + BlockProducer + Nonce + Sync,
	{
		...
		for tx in pending {
			...
			//println!("transaction, v_shield_outputs.len = {0}", transaction.transaction.unsigned.v_shielded_output().len());
	...
	{
		//println!("In seal_and_import_block_internally");
		//self.debug_println(&block);
		{
	...
	fn prepare_work(&self, block: ClosedBlock, original_work_hash: Option<H256>) {
		let (work, is_new) = {
			...
			let (work, is_new) = if last_work_hash.map_or(true, |h| h != block_hash) {
				...
				let pow_hash =  block_header.pow_hash();
				(Some((pow_hash, *block_header.difficulty(), block_header.number())), is_new)
			} else {
				(None, false)
			};
			trace!(
	...
	/*fn debug_println(&self, block: &ClosedBlock){
...
impl miner::MinerService for Miner {
	...
	where
		C: ChainInfo + Nonce + Sync + TransactionInfo,
	{
		let chain_info = chain.chain_info();
	...
	fn work_package<C>(&self, chain: &C) -> Option<(H512, BlockNumber, u64, U256)> where
		C: BlockChain + CallContract + BlockProducer + SealedBlockImporter + Nonce + Sync,
	{
		if self.engine.seals_internally().is_some() {
			return None;
		...
		self.sealing.lock().queue.use_last_ref().map(|b| {
			let header = &b.header;
			(header.pow_hash(), header.number(), header.timestamp(), *header.difficulty())
		})
	}

	// Note used for external submission (PoW) and internally by sealing engines.
	fn submit_seal(&self, pow_hash: H512, seal: Vec<Bytes>) -> Result<SealedBlock, Error> {
		let result =
			if let Some(b) = self.sealing.lock().queue.get_used_if(
				...
				|b| &b.header.pow_hash() == &pow_hash
			) {
				trace!(target: "miner", "Submitted block {} with seal {:?}", pow_hash,  seal);
...
mod tests {
	...
	use types::transaction::{Transaction, CONVERSION_FACTOR};

	use rand::{OsRng, Rand};
	use blockchain::wallet::wallet_types::{
		SaplingExtendedFullViewingKey, SaplingExtendedSpendingKey,
	};
	use blockchain::wallet::transaction_builder::TransactionBuilder;
	...
	fn create_private(action: Action) -> Transaction {
		...
		if let Action::PrivateCall(address) = action {
			builder.set_public_output(address, &U256::from(0));
		}
	...
	fn public_to_private_transaction() -> SignedTransaction {
		let extsk = SaplingExtendedSpendingKey::master(&[]);
		let extfvk = SaplingExtendedFullViewingKey::from(&extsk);
		let ovk = extfvk.fvk.ovk;
		let to = extfvk.default_address().unwrap().1;
	...
	fn privacy_transaction() -> SignedTransaction {
		let mut transaction = create_private(Action::PrivateToPrivate);
		transaction.sign_for_private(TEST_CHAIN_ID)
	}

	fn unshield_transaction() -> SignedTransaction {
		let mut transaction = create_private(Action::PrivateCall(Address::from("0000000000000000000000000000000000000005")));
		transaction.sign_for_private(TEST_CHAIN_ID)
	}

	fn transaction_list() -> Vec<SignedTransaction> {
		return vec![transaction(), privacy_transaction(), public_to_private_transaction(), unshield_transaction()];
	}
	...
	fn should_import_external_transaction() {
		for sign_transaction in transaction_list() {
			let transaction = sign_transaction.into();
			let client = TestBlockChainClient::default();
			let miner = miner();
			let best_block = 0;
			// when
	...
	fn should_mine_if_fetch_work_request() {
		...
		impl NotifyWork for DummyNotifyWork {
			fn notify(&self, _pow_hash: H512, _difficulty: U256, _number: u64) { }
		}
