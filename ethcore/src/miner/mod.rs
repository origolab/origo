...
use client::{
	...
	AccountData, Nonce, TransactionInfo,
};
use error::Error;
use state::StateInfo;
...
pub trait MinerService : Send + Sync {
	...
	fn work_package<C>(&self, chain: &C) -> Option<(H512, BlockNumber, u64, U256)>
	...
	fn next_nonce<C>(&self, chain: &C, address: &Address) -> U256
		where C: Nonce + TransactionInfo + Sync;
	...
	fn ready_transactions<C>(&self, chain: &C, max_len: usize, ordering: PendingOrdering) -> Vec<Arc<VerifiedTransaction>>
		where C: ChainInfo + Nonce + TransactionInfo + Sync;
