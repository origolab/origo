...
use ethereum_types::{H256, clean_0x, U256, H512};
use equihash;
...
impl SubmitPayload {
	fn from_args(payload: Vec<String>) -> Result<Self, PayloadError> {
		...
		let pow_hash = match clean_0x(&payload[0]).parse::<H512>() {
		...
		let nonce = match clean_0x(&payload[1]).parse::<H256>() {
			Ok(nonce) => nonce,
			Err(e) => {
				warn!(target: "stratum", "submit_work ({}): invalid nonce ({:?})",  &payload[1], e);
				return Err(PayloadError::InvalidNonce(payload[1].clone()));
			}
		};

		let sol = match FromHex::from_hex(clean_0x(&payload[2])){
			Ok(value) => value,
			Err(err) => {
				warn!(target: "stratum", "submit_work ({}): invalid solution ({:?})",  &payload[2], err);
				return Err(PayloadError::InvalidSolution(payload[2].clone()))
			},
		};

		let solution = match EquihashSolution::try_from(sol){
			Ok(s) => s,
			Err(_e) => {
				warn!(target: "stratum", "submit_work ({}): invalid solution length",  &payload[2]);
				return Err(PayloadError::InvalidSolution(payload[2].clone()))
			},
		};
...
impl JobDispatcher for StratumJobDispatcher {
	...
	fn submit(&self, payload: Vec<String>) -> Result<(), StratumServiceError> {
		...
		trace!(
			...
			payload.nonce,
			payload.pow_hash,
			payload.equihash_solution,
		);

		self.with_core_result(|client, miner| {
			let seal = vec![encode(&payload.nonce),encode(&payload.equihash_solution)];
...
impl StratumJobDispatcher {
	...
	fn payload(&self, pow_hash: H512, difficulty: U256, number: u64) -> String {
