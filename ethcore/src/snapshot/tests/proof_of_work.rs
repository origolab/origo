...
fn chunk_and_restore(amount: u64) {
	...
	let bc = BlockChain::new(Default::default(), genesis.encoded().raw(), old_db.clone(), None);
	...
	let new_chain = BlockChain::new(Default::default(), genesis.encoded().raw(), new_db.clone(), None);
	...
	// and test it.
	let new_chain = BlockChain::new(Default::default(), genesis.encoded().raw(), new_db, None);
...
fn checks_flag() {
	...
	let engine = ::spec::Spec::new_test().engine;
	let chain = BlockChain::new(Default::default(), genesis.last().encoded().raw(), db.clone(), None);
