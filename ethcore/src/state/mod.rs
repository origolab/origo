...
impl<B: Backend> State<B> {
	...
	///	Substracts 'by' from the balance of 'from'
	pub fn transfer_balance_from_public_to_private(&mut self, from: &Address, to: &Address, by: &U256, mut cleanup_mode: CleanupMode) -> TrieResult<()> {
		self.sub_balance(from, by, &mut cleanup_mode)?;
		Ok(())
	}
	/// Adds 'by' to that of 'to'
	pub fn transfer_balance_from_private_to_public(&mut self, from: &Address, to: &Address, by: &U256, mut cleanup_mode: CleanupMode) -> TrieResult<()> {
