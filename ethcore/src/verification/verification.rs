...
use db::keys::CommitmentTreeSerialization;
...
fn verify_parent(header: &Header, parent: &Header, engine: &EthEngine) -> Result<(), Error> {
	...
	if !engine.is_timestamp_valid(header.timestamp(), parent.timestamp()) {
		...
		let min = now
			.checked_add(Duration::from_secs(parent.timestamp().saturating_add(1)))
			.ok_or(BlockError::TimestampOverflow)?;
		let found = now
			.checked_add(Duration::from_secs(header.timestamp()))
			.ok_or(BlockError::TimestampOverflow)?;
...
mod tests {
	...
	impl BlockProvider for TestBlockChain {
		...
		fn nullifier_transaction(&self, _nullifier: &U256) -> Option<H256> {
			unimplemented!()
		}

		fn commitment_root_block(&self, _root: &Node) -> Option<H256> {
			unimplemented!()
		}

		fn block_commitment_ser(&self, _hash: &H256) -> Option<CommitmentTreeSerialization> {
			unimplemented!()
		}
	...
	fn test_verify_block() {
		...
		let tr1 = Transaction::create_private(vec![0.into()], U256::zero(), Action::PrivateToPrivate)
			.sign_for_private(2);
