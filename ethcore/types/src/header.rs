...
use hash::{KECCAK_NULL_RLP, KECCAK_EMPTY_LIST_RLP, keccak, keccak_512};
use heapsize::HeapSizeOf;
use ethereum_types::{H256, U256, Address, Bloom, H512};
use bytes::Bytes;
...
pub struct Header {
	...
	//  For equihash seal is nonce + solution
	seal: Vec<Bytes>,
...
impl PartialEq for Header {
	fn eq(&self, c: &Header) -> bool {
		...
		self.seal == c.seal //&&
		//self.hash_final_cm_root == c.hash_final_cm_root
	}
}
...
impl Default for Header {
	fn default() -> Self {
		Header {
			...
			//hash_final_cm_root: H256::from(0),
		}
	}
}
...
impl Header {
	...
	//pub fn hash_final_cm_root(&self) -> H256 { self.hash_final_cm_root }
	...
	/// Get the hash of the header excluding the seal
	pub fn pow_hash(&self) -> H512 {
		let mut result = [0u8; 64];
		keccak_512(&self.rlp(Seal::Without), &mut result);
		H512(result)
	}
	...
	fn stream_rlp(&self, s: &mut RlpStream, with_seal: Seal) {
		...
		//s.append(&self.hash_final_cm_root);
...
impl Decodable for Header {
	fn decode(r: &Rlp) -> Result<Self, DecoderError> {
		let mut blockheader = Header {
			...
			seal: vec![],
			//hash_final_cm_root: r.val_at(13)?,
...
mod tests {
	...
	fn test_header_seal_fields() {
		...
		let header_rlp = "".from_hex().unwrap();
		let solution = "a0a0349d8c3df71f1a48a9df7d03fd5f14aeee7d91332c009ecaff0a71ead405bd".from_hex().unwrap();
		let solution_decoded = "a0349d8c3df71f1a48a9df7d03fd5f14aeee7d91332c009ecaff0a71ead405bd".from_hex().unwrap();
		...
		assert_eq!(seal_fields[0], nonce);
		assert_eq!(seal_fields[1], solution);

		let decoded_seal = header.decode_seal::<Vec<_>>().unwrap();
		assert_eq!(decoded_seal.len(), 2);
		assert_eq!(decoded_seal[0], &*nonce_decoded);
		assert_eq!(decoded_seal[1], &*solution_decoded);
	}
