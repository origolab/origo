...
impl<'a> BlockView<'a> {
	...
	pub fn localized_private_txs(&self) -> Vec<LocalizedTransaction> {
		let header = self.header_view();
		let block_hash = header.hash();
		let block_number = header.number();
		self.transactions()
...
mod tests {
	...
	use transaction::{transaction, Transaction, UnverifiedTransaction};
	...
	#[test]
	fn test_private_txs_in_block() {
		let t = Transaction::create_private(vec![0], 42.into(), transaction::Action::PrivateToPrivate)
			.sign_for_private(0);
		let t1 = Transaction {
			nonce: 0.into(),
			gas_price: 0.into(),
		...
		.invalid_sign();
		let transactions: Vec<UnverifiedTransaction> = vec![t.into(), t1];
		let block = Block {
			transactions,
			..Default::default()
		};
		let rlp = block.rlp_bytes();
		let view = view!(BlockView, &rlp);
		assert_eq!(view.localized_transactions().len(), 2);
		assert_eq!(view.localized_private_txs().len(), 1);
	}
}
