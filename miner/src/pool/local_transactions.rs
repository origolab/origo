...
mod tests {
	...
	#[test]
	fn should_add_pr_transaction_as_pending() {
		// given
		let mut list = LocalTransactionsList::default();
		let tx1 = new_private_tx(10);
		let tx2 = new_private_tx(20);

		// when
		list.added(&tx1, None);
		list.added(&tx2, None);

		assert!(tx1.is_private());
		// then
		assert!(list.contains(tx1.hash()));
		assert!(list.contains(tx2.hash()));
		let statuses = list.all_transactions().values().cloned().collect::<Vec<Status>>();
	...
	/// create a origo private transaction
	fn new_private_tx<T: Into<U256>>(nonce: T) -> Arc<Transaction> {
		let keypair = Random.generate().unwrap();
		let signed = transaction::Transaction {
			action: transaction::Action::Create,
			value: U256::from(100),
