...
use pool::{self, replace, scoring, verifier, client, ready, listener, PrioritizationStrategy, PendingOrdering, PendingSettings, VerifiedTransaction};
use pool::local_transactions::LocalTransactionsList;
use std::ptr::null;
use txpool::scoring::Scoring;
...
/// NullifierPending struct is used to check the nullifier confilict for
/// private transaction
#[derive(Debug)]
struct NullifierPending {
	nullifier_hash_map: RwLock<HashMap<U256, H256>>,
}

impl NullifierPending {
	fn new() -> Self {
		NullifierPending {
			nullifier_hash_map: RwLock::new(HashMap::new()),
		}
	}

	/// Return the transaction hashes which conflict with the nullifiers
	fn conflict_transaction_hashes(&self, nullifiers: &HashSet<U256>) -> HashSet<H256> {
		let mut conflict_hashes = HashSet::new();
		for nullifier in nullifiers {
			if let Some(hash) = self.nullifier_hash_map.read().get(nullifier) {
				conflict_hashes.insert(hash.clone());
			}
		}
		conflict_hashes
	}

	/// Insert the new transaction and the nullifiers info.
	fn insert(&self, hash: H256, nullifiers: &HashSet<U256>) {
		for nullifier in nullifiers {
			self.nullifier_hash_map.write().insert(nullifier.clone(), hash.clone());
		}
	}

	/// Remove the specific transaction and nullifier info.
	fn remove(&self, nullifiers: &HashSet<U256>) {
		for nullifier in nullifiers {
			self.nullifier_hash_map.write().remove(nullifier);
		}
	}

	fn size(&self) -> usize {
		self.nullifier_hash_map.read().len()
	}

	fn clear(&self) {
		self.nullifier_hash_map.write().clear();
	}
}
...
pub struct TransactionQueue {
	...
	// check the nullifier for the private transaction
	nullifier_pending: NullifierPending,
	sender: Option<Arc<Mutex<Sender<SyncTransaction>>>>,
}
...
impl TransactionQueue {
	...
	pub fn new(
		...
		TransactionQueue {
			...
			nullifier_pending: NullifierPending::new(),
			sender,
		}
	}
	...
	pub fn import<C: client::Client + client::NonceClient + Clone>(
		...
		let results = transactions
			...
			.map(|transaction| {
				...
				let mut transaction_nullifiers = HashSet::new();
				let mut replaced_transaction_hashes = HashSet::new();

				let imported = verifier
					.verify_transaction(transaction)
					.and_then(|verified| {
						self.check_transaction_nullifier_conflict(&verified,
														   &mut transaction_nullifiers,
														   &mut replaced_transaction_hashes)?;
						self.pool.write().import(verified, &mut replace).map_err(convert_error)
					});

				match imported {
					Ok(tx) => {
						self.remove(replaced_transaction_hashes.iter(), false);
						self.nullifier_pending.insert(hash, &transaction_nullifiers);
						if self.sender.is_some() {
							let s = self.sender.clone().unwrap();
							let ss = s.lock();
							ss.send(SyncTransaction::InPool(tx.transaction.clone()));
						}
						Ok(())
					}
					Err(err) => {
	...
	/// Check the private transaction nullifier conflict.
	pub fn check_transaction_nullifier_conflict(&self, verified: &VerifiedTransaction,
										 transaction_nullifiers: &mut HashSet<U256>,
										 replaced_transaction_hashs: &mut HashSet<H256>) -> Result<(), transaction::Error> {
		*transaction_nullifiers = verified.transaction.get_nullifier_set();
		*replaced_transaction_hashs = self.nullifier_pending.conflict_transaction_hashes(&transaction_nullifiers);
		for hash in replaced_transaction_hashs.iter() {
			if let Some(transaction) = self.pool.read().find(hash) {
				if !self.pool.read().scoring().should_replace_conflict(&transaction, &verified) {
	...
	pub fn pending<C>(
		...
		C: client::NonceClient + client::NullifierClient,
	{
	...
	pub fn cull<C: client::NonceClient + Clone + client::NullifierClient>(
		&self,
		client: C,
	) {
	...
	pub fn next_nonce<C: client::NonceClient + client::NullifierClient>(
		&self,
		client: C,
		address: &Address,
	) -> Option<U256> {
	...
	pub fn remove<'a, T: IntoIterator<Item = &'a H256>>(
		...
		let mut results: Vec<Option<Arc<pool::VerifiedTransaction>>> = Vec::new();
		for hash in hashes {
			if let Some(transaction) = pool.find(hash) {
				self.nullifier_pending.remove(&transaction.transaction.get_nullifier_set());
			}
			results.push(pool.remove(hash, is_invalid));
		}
...
mod tests {
	...
	fn should_get_pending_transactions() {
		let queue = TransactionQueue::new(txpool::Options::default(), verifier::Options::default(), PrioritizationStrategy::GasPriceOnly, None);
