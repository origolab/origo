...
use super::client::{NullifierClient, NonceClient};
...
impl<C: NonceClient + NullifierClient> txpool::Ready<VerifiedTransaction> for State<C> {
	fn is_ready(&mut self, tx: &VerifiedTransaction) -> txpool::Readiness {
		if tx.transaction.action.is_input_private() {
			for spend in tx.transaction.v_shielded_spend().iter() {
				if self.state.nullifier_exists(&U256::from(spend.nullifier)) {
...
mod tests {
	...
	#[test]
	fn should_return_correct_state_for_privacy() {
		let tx1 = transaction::Transaction::create_private(vec![0.into()], 0.into(), transaction::Action::PrivateToPrivate).sign_for_private(0);
		let tx2 = transaction::Transaction::create_private(vec![1.into()], 124.into(),  transaction::Action::PrivateToPrivate).sign_for_private(0);
		let tx3 = transaction::Transaction::create_private(vec![2.into()], 123.into(),  transaction::Action::PrivateToPrivate).sign_for_private(0);
		let (tx1, tx2, tx3) = (tx1.verified(), tx2.verified(), tx3.verified());
		assert_eq!(State::new(TestClient::new(), None, None).is_ready(&tx1), txpool::Readiness::Ready);
		assert_eq!(State::new(TestClient::new(), None, None).is_ready(&tx2), txpool::Readiness::Ready);
		assert_eq!(State::new(TestClient::new(), None, None).is_ready(&tx3), txpool::Readiness::Ready);

		assert_eq!(State::new(TestClient::new().with_nullifier(vec![U256::from([0u8; 32])]), None, None).is_ready(&tx1), txpool::Readiness::Stale);
		assert_eq!(State::new(TestClient::new().with_nullifier(vec![U256::from([0u8; 32])]), None, None).is_ready(&tx2), txpool::Readiness::Ready);
		assert_eq!(State::new(TestClient::new().with_nullifier(vec![U256::from([0u8; 32])]), None, None).is_ready(&tx3), txpool::Readiness::Ready);
	}
