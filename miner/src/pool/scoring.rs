...
impl NonceAndGasPrice {
	...
	/// Decide if the new transaction should replace the old transaction.
	/// When there is conflict between the nullifiers.
	pub fn should_replace_conflict(&self, old: &VerifiedTransaction, new: &VerifiedTransaction) -> bool {
...
mod tests {
	...
	fn should_calculate_score_correctly() {
		...
		assert_eq!(scores, vec![32768000000u64.into(), 0.into(), 0.into()]);
		scoring.update_scores(&transactions, &mut *scores, scoring::Change::InsertedAt(1));
		assert_eq!(scores, vec![32768000000u64.into(), 1024000000.into(), 0.into()]);
		scoring.update_scores(&transactions, &mut *scores, scoring::Change::InsertedAt(2));
		assert_eq!(scores, vec![32768000000u64.into(), 1024000000.into(), 1000000.into()]);
		...
		assert_eq!(scores, vec![32768000000u64.into(), 128000000.into(), 125000.into()]);
	}
}
