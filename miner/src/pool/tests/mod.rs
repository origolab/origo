...
use self::tx::{Tx, TxExt, PairExt, PrivateTx};
use self::client::TestClient;

use rand::{OsRng, Rand};
use ethcore_blockchain::wallet::wallet_types::{
	SaplingExtendedFullViewingKey, SaplingExtendedSpendingKey,
};
use ethcore_blockchain::wallet::transaction_builder::TransactionBuilder;
use sapling_crypto::{
	jubjub::fs::Fs,
	redjubjub::Signature,
};
...
use ff::PrimeField;
use types::transaction::{Action,Transaction,UnverifiedTransaction,SignedTransaction, PrivateTransaction, CONVERSION_FACTOR};
...
fn new_queue() -> TransactionQueue {
	TransactionQueue::new(
		...
		verifier::Options {
			minimal_gas_price: 1_000_000.into(),
...
fn public_to_private_transaction(gas_price: U256, nonce: U256) -> SignedTransaction {
	let extsk = SaplingExtendedSpendingKey::master(&[]);
	let extfvk = SaplingExtendedFullViewingKey::from(&extsk);
	let ovk = extfvk.fvk.ovk;
	let to = extfvk.default_address().unwrap().1;
	let mut builder = TransactionBuilder::new(gas_price, nonce, U256::from(21000), vec![], TEST_CHAIN_ID);
	builder.set_public_input(U256::from(0));

	// Add output
	assert!(builder.add_sapling_output(ovk, to, &U256::from(0), "haha").is_ok());
	let result = builder.build();
	assert!(result.is_ok());
	let mut tx = result.unwrap();
	let keypair = Random.generate().unwrap();
	tx.sign(keypair.secret(), Some(TEST_CHAIN_ID))
}

fn add_private_input(builder: &mut TransactionBuilder, value: u64) {
	...
	// Add input,
	let note1 = to.create_note(value, Fs::rand(&mut rng), &JUBJUB).unwrap();
...
fn private_to_public_transaction(gas_price: U256, value: u64) -> SignedTransaction {
	let mut builder = TransactionBuilder::new(gas_price.into(), U256::from(0), U256::from(21000), vec![], TEST_CHAIN_ID);
	// 21000 * gas_price is used to pay for transaction fee.
	add_private_input(&mut builder, value + 21000 * gas_price.low_u64());
	// Set output.
	builder.set_public_output(Address::from("0000000000000000000000000000000000000005"), &(U256::from(value) * CONVERSION_FACTOR));
	let result = builder.build();
	assert!(result.is_ok());
	result.unwrap().sign_for_private(TEST_CHAIN_ID)
}

fn create_private(gas_price: u64) -> SignedTransaction {
	let mut rng = OsRng::new().expect("should be able to construct RNG");
	...
	let mut builder = TransactionBuilder::new(gas_price.into(), U256::from(0), U256::from(21000), vec![], TEST_CHAIN_ID);
	add_private_input(&mut builder, 300 + 21000 * gas_price / CONVERSION_FACTOR.low_u64());
...
fn create_conflict_nullifier_transaction_pair(
	gas_price1: u64,
	gas_price2: u64,
	action1: Action,
	action2: Action,
) -> (Transaction, Transaction) {
	let mut rng1 = OsRng::new().expect("should be able to construct RNG");

	let extsk1 = SaplingExtendedSpendingKey::master(&[]);
	let extfvk1 = SaplingExtendedFullViewingKey::from(&extsk1);
	let ovk1 = extfvk1.fvk.ovk;
	let to1 = extfvk1.default_address().unwrap().1;

	let mut builder1 = TransactionBuilder::new(gas_price1.into(), U256::from(0), U256::from(21000), vec![], TEST_CHAIN_ID);

	// Add input
	let note1 = to1.create_note(100000, Fs::rand(&mut rng1), &JUBJUB).unwrap();
	let mut tree1 = CommitmentTree::new();
	let cm1 = Node::new(note1.cm(&JUBJUB).into_repr());

	tree1.append(cm1).unwrap();
	let inc_tree1 = IncrementalWitness::from_tree(&tree1);
	let witness1 = inc_tree1.path().unwrap();
	assert!(builder1
		.add_sapling_spend(
			extsk1.expsk,
			to1.diversifier,
	...
	if let Action::PrivateCall(address) = action1 {
		builder1.set_public_output(address, &U256::from(0));
	} else {
		// Add output
		assert!(builder1.add_sapling_output(ovk1, to1, &(U256::from(200) * CONVERSION_FACTOR), "haha").is_ok());
	}

	let result1 = builder1.build();
	assert!(result1.is_ok());
	let mut tx1 = result1.unwrap();

	let mut rng2 = OsRng::new().expect("should be able to construct RNG");

	let extsk2 = SaplingExtendedSpendingKey::master(&[]);
	let extfvk2 = SaplingExtendedFullViewingKey::from(&extsk2);
	let ovk2 = extfvk2.fvk.ovk;
	let to2 = extfvk2.default_address().unwrap().1;

	let mut builder2 = TransactionBuilder::new(gas_price2.into(), U256::from(0), U256::from(21000), vec![], TEST_CHAIN_ID);

	// Add input
	let note2 = to2.create_note(100000, Fs::rand(&mut rng2), &JUBJUB).unwrap();
	let mut tree2 = CommitmentTree::new();
	let cm2 = Node::new(note2.cm(&JUBJUB).into_repr());

	tree2.append(cm2).unwrap();
	let inc_tree2 = IncrementalWitness::from_tree(&tree2);
	let witness2 = inc_tree2.path().unwrap();
	let inc_tree1 = IncrementalWitness::from_tree(&tree1);
	let witness1 = inc_tree1.path().unwrap();
	assert!(builder2
		.add_sapling_spend(
	...
	// Add output
	assert!(builder2.add_sapling_output(ovk2, to2, &(U256::from(200) * CONVERSION_FACTOR), "haha").is_ok());

	if let Action::PrivateCall(address) = action2 {
		builder2.set_public_output(address, &U256::from(0))
	}

	let result2 = builder2.build();
	assert!(result2.is_ok());
	let mut tx2 = result2.unwrap();

	(tx1, tx2)
}

#[test]
fn test_invalid_spend_description() {
	...
	let mut tx = create_private(2_000_000).clone();
	for spend in &mut tx.transaction.unsigned.private.as_mut().unwrap().spends {
		spend.spend_auth_sig = Some(Signature::new());
	}
	let r = txq.import(TestClient::new(), vec![tx].local());
	assert_eq!(r, vec![Err(transaction::Error::InvalidSpendDescription)]);
}

#[test]
fn test_invalid_output_description() {
	let txq = TransactionQueue::new(
		txpool::Options {
			max_count: 3,
			max_per_sender: 1,
	...
	let mut tx = create_private(2_000_000);
	for output in &mut tx.transaction.unsigned.private.as_mut().unwrap().outputs {
		output.cmu = Fr::from_repr(FrRepr::default()).unwrap();
	}
	let r = txq.import(TestClient::new(), vec![tx].local());
	assert!(r[0].is_err());
}
...
#[test]
fn test_invalid_balancing_value_or_binding_sig() {
	let txq = TransactionQueue::new(
		txpool::Options {
			max_count: 3,
			max_per_sender: 1,
	...
	let mut tx = create_private(2_000_000).clone();
	tx.transaction.unsigned.private.as_mut().unwrap().spends = vec![];
	// when
	let r = txq.import(TestClient::new(), vec![tx].local());
	assert_eq!(r, vec![Err(transaction::Error::InvalidBalancingValueOrBindingSig)]);
}
...
#[test]
fn should_return_correct_nonces_when_dropped_because_of_limit_for_shield() {
	// given
	let txq = TransactionQueue::new(
		txpool::Options {
			max_count: 3,
		...
		verifier::Options {
			minimal_gas_price: 2000000.into(),
			block_gas_limit: 1_000_000.into(),
			tx_gas_limit: 1_000_000.into(),
			no_early_reject: false,
		},
	...
	// when
	let tx1 = public_to_private_transaction(U256::from(10000000),  U256::from(123));
	let tx2 = public_to_private_transaction(U256::from(2000000),  U256::from(123));

	let sender = tx2.sender();

	let tx3 = public_to_private_transaction(U256::from(1000000),  U256::from(123));
	let tx4 = public_to_private_transaction(U256::from(3000000),  U256::from(123));
	...
	// then
	assert_eq!(res, vec![
		Err(transaction::Error::InsufficientBalance { balance: U256::from(63_100_000_000u64), cost: U256::from(210_000_000_000u64)}),
		Ok(())]);
	assert_eq!(res2, vec![
		// The error here indicates reaching the limit
		// and minimal effective gas price taken into account.
		Err(transaction::Error::InsufficientGasPrice { minimal: 2000000.into(), got: 1000000.into() }),
		Ok(())
	]);
...
#[test]
fn should_return_correct_nonces_when_dropped_because_of_limit_for_privacy() {
	// given
	let txq = TransactionQueue::new(
		txpool::Options {
			max_count: 3,
	...
	let (mut tx1, mut tx2) = create_conflict_nullifier_transaction_pair(2_000_000, 2_000_000, Action::PrivateToPrivate, Action::PrivateToPrivate);

	// when
	let r1 = txq.import(TestClient::new(), vec![tx1.sign_for_private(TEST_CHAIN_ID)].retracted());
	let r2 = txq.import(TestClient::new(), vec![tx2.sign_for_private(TEST_CHAIN_ID)].local());
	assert_eq!(r1, vec![Ok(())]);
	assert_eq!(r2, vec![Err(transaction::Error::ConflictNullifier)]);
	assert_eq!(txq.status().status.transaction_count, 1);

	// when
	let tx1 = create_private(2_000_000);
	let tx2 = create_private(2_000_000);

	let sender = tx2.sender();

	let tx3 = create_private(1_000_000);
	let tx4 = create_private(3_000_000);
...
#[test]
fn check_transaction_nullifier_conflict_for_private() {
	let txq = TransactionQueue::new(
		txpool::Options {
			max_count: 3,
			max_per_sender: 1,
	...
	let (mut tx1, mut tx3) = create_conflict_nullifier_transaction_pair(2_000_000, 2_000_000, Action::PrivateToPrivate, Action::PrivateToPrivate);
	let (mut tx2, mut tx4) = create_conflict_nullifier_transaction_pair(3_000_000, 4_000_000, Action::PrivateToPrivate, Action::PrivateToPrivate);
	let res = txq.import(TestClient::new(), vec![tx1.sign_for_private(TEST_CHAIN_ID), tx2.sign_for_private(TEST_CHAIN_ID)].local());
	assert_eq!(res, vec![Ok(()), Ok(())]);
	assert_eq!(txq.status().status.transaction_count, 2);

	// Can't replace tx1 because of low gas price.
	let res3 = txq.import(TestClient::new(), vec![tx3.sign_for_private(TEST_CHAIN_ID)].local());
	assert_eq!(res3, vec![Err(transaction::Error::ConflictNullifier)]);

	// Replace tx2 because of high gas price.
	let res4 = txq.import(TestClient::new(), vec![tx4.sign_for_private(TEST_CHAIN_ID)].local());
	assert_eq!(res4, vec![Ok(())]);
	assert_eq!(txq.status().status.transaction_count, 2);
}

#[test]
fn check_transaction_nullifier_remove_for_private() {
	// given
	let txq = TransactionQueue::new(
		txpool::Options {
			max_count: 3,
...
#[test]
fn should_return_correct_nonces_when_dropped_because_of_limit_for_unshield() {
	// given
	let txq = TransactionQueue::new(
		txpool::Options {
			max_count: 3,
	...
	let (mut tx1, mut tx2) = create_conflict_nullifier_transaction_pair(
		2_000_000,
		2_000_000,
	...
	// when
	let tx1 = private_to_public_transaction(2_000_000.into(), 0);
	let tx2 = private_to_public_transaction(2_000_000.into(), 0);

	let sender = tx2.sender();

	let tx3 = private_to_public_transaction(1_000_000.into(), 0);
	let tx4 = private_to_public_transaction(3_000_000.into(), 0);
...
#[test]
fn check_public_and_private_transaction_import() {
	let txq = TransactionQueue::new(
		txpool::Options {
			max_count: 3,
			max_per_sender: 1,
	...
	// when
	let tx1 = create_private(2_000_000);
	let tx2 = Tx::gas_price(2_000_000).signed();
	let res = txq.import(TestClient::new(), vec![tx1, tx2].local());
...
#[test]
fn should_move_all_transactions_from_future_for_private() {
	// given
	let txq = new_queue();
	let mut tx = create_private(0);
	let tx2 = create_private(1_000_000);
	let nullifier = U256::from(tx.private.as_ref().unwrap().spends[0].nullifier).clone();
	let tx2 = create_private(1_000_000);
	let nullifier2 = U256::from(tx2.private.as_ref().unwrap().spends[0].nullifier).clone();
	...
	assert_eq!(res, vec![Ok(())]);
	assert_eq!(txq.status().status.transaction_count, 2);
	let top = txq.pending(client.clone(), PendingSettings::all_prioritized(0, 0));
	assert_eq!(top.len(), 2);

	let top = txq.pending(TestClient::new().with_nullifier(vec![nullifier, nullifier2]), PendingSettings::all_prioritized(1, 1));
	assert_eq!(top.len(), 0);

	let client = TestClient::new().with_nonce(0);
	let top = txq.pending(client, PendingSettings::all_prioritized(2, 1));
	assert_eq!(top.len(), 2);
}
...
fn should_move_all_transactions_from_future() {
	...
	let top = txq.pending(TestClient::new().with_nonce(124), PendingSettings::all_prioritized(1, 1));
	assert_eq!(top.len(), 0);
}
...
fn should_not_import_transaction_below_min_gas_price_threshold_if_external() {
	...
	txq.set_verifier_options(verifier::Options {
		minimal_gas_price: 3_000_000.into(),
		..Default::default()
	});
	...
	assert_eq!(res, vec![Err(transaction::Error::InsufficientGasPrice {
		minimal: U256::from(3_000_000),
		got: U256::from(1_000_000),
	})]);
	assert_eq!(txq.status().status.transaction_count, 0);
}
...
fn should_clear_queue() {
	...
	// add
	let mut tx_vec = txs.local().into_vec();
	tx_vec.push(create_private(0).local());
	txq.import(TestClient::new(), tx_vec);
	assert_eq!(txq.status().status.transaction_count, 3);
...
#[test]
fn should_not_insert_same_private_transaction_twice() {
	// given
	let txq = new_queue();
	let tx = create_private(1_000_000);
	let res = txq.import(TestClient::new(), vec![tx.clone().local()]);
	assert_eq!(res, vec![Ok(())]);
	assert_eq!(txq.status().status.transaction_count, 1);
...
fn should_not_replace_same_transaction_if_the_fee_is_less_than_minimal_bump() {
	// given
	let txq = new_queue();
	let (tx, tx2) = Tx::gas_price(20_000_000).signed_replacement();
	let (tx3, tx4) = Tx::gas_price(1_000_000).signed_replacement();
	let client = TestClient::new().with_balance(1_000_000_000_000_000u64);
	...
	assert_eq!(txq.status().status.transaction_count, 2);
	assert_eq!(txq.pending(client.clone(), PendingSettings::all_prioritized(0, 0))[0].signed().gas_price, U256::from(20_000_000));
	assert_eq!(txq.pending(client.clone(), PendingSettings::all_prioritized(0, 0))[1].signed().gas_price, U256::from(2_000_000));
}
...
fn should_include_local_transaction_to_a_full_pool() {
	...
	let tx1 = Tx::gas_price(10_000_000_000u64).signed().unverified();
	let tx2 = Tx::gas_price(1_000_000).signed().local();

	let res = txq.import(TestClient::new().with_balance(1_000_000_000_000_000u64), vec![tx1]);
	assert_eq!(res, vec![Ok(())]);
	assert_eq!(txq.status().status.transaction_count, 1);
...
fn should_avoid_verifying_transaction_already_in_pool() {
	...
	let client = TestClient::new().with_balance(100_000_000_000u64);
	let tx1 = Tx::gas_price(2_000_000).signed().unverified();
...
fn should_avoid_reverifying_recently_rejected_transactions() {
	...
	let client = TestClient::new();
	let tx1 = Tx::gas_price(10_000_000_000).signed().unverified();

	let res = txq.import(client.clone(), vec![tx1.clone()]);
	assert_eq!(res, vec![Err(transaction::Error::InsufficientBalance {
		balance: 63100000000u64.into(),
		cost: 210001000000000u64.into(),
	})]);
	assert_eq!(txq.status().status.transaction_count, 0);
	assert!(client.was_verification_triggered());
