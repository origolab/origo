...
usage! {
	{
		...
		CMD cmd_account
		{
			...
			CMD cmd_account_new {
				"Create a new account (and its associated key) for the given --chain (default: medietas)",
			}

			CMD cmd_account_list {
				"List existing accounts of the given --chain (default: medietas)",
			}

			CMD cmd_account_import
			{
				"Import accounts from JSON UTC keystore files to the specified --chain (default medietas)",
		...
		CMD cmd_wallet
		{
			...
			{
				"Import wallet into the given --chain (default: medietas)",
		...
		CMD cmd_import
		{
			"Import blockchain data from a file to the given --chain database (default: medietas)",
		...
		CMD cmd_export
		{
			...
			{
				"Export the blockchain blocks from the given --chain database (default: medietas) into a file. This command requires the chain to be synced with --fat-db on.",
			...
			{
				"Export the blockchain state from the given --chain (default: medietas) into a file. This command requires the chain to be synced with --fat-db on.",
		...
		CMD cmd_signer
		{
			...
			CMD cmd_signer_new_token {
				"Generate a new signer-authentication token for the given --chain (default: medietas)",
			}

			CMD cmd_signer_list {
				"List the signer-authentication tokens from given --chain (default: medietas)",
			}
		...
		CMD cmd_snapshot
		{
			"Make a snapshot of the database of the given --chain (default: medietas)",
		...
		CMD cmd_restore
		{
			"Restore the database of the given --chain (default: medietas) from a snapshot file",
		...
		CMD cmd_db
		{
			...
			CMD cmd_db_kill {
				"Clean the database of the given --chain (default: medietas)",
			}
		...
		{
			"Print the hashed light clients headers of the given --chain (default: medietas) in a JSON format. To be used as hardcoded headers in a genesis file.",
		}
	...
	}
	{
		...
		["Operating Options"]
			...
			"--force-direct",
			"Run the originally installed version of Origo, ignoring any updates that have since been installed.",

			ARG arg_mode: (String) = "last", or |c: &Config| c.parity.as_ref()?.mode.clone(),
			"--mode=[MODE]",
			"Set the operating mode. MODE can be one of: last - Uses the last-used mode, active if none; active - Origo continuously syncs the chain; passive - Origo syncs initially, then sleeps and wakes regularly to resync; dark - Origo syncs only when the JSON-RPC is active; offline - Origo doesn't sync.",
			...
			ARG arg_chain: (String) = "medietas", or |c: &Config| c.parity.as_ref()?.chain.clone(),
			"--chain=[CHAIN]",
			"Specify the blockchain type. CHAIN may be either a JSON chain specification file or ethereum, classic, poacore, tobalaba, expanse, musicoin, ellaism, mix, callisto, morden, ropsten, kovan, rinkeby, goerli, kotti, poasokol, medietas, testnet, or dev.",

			ARG arg_keys_path: (String) = "$BASE/keys", or |c: &Config| c.parity.as_ref()?.keys_path.clone(),
			"--keys-path=[PATH]",
			"Specify the path for JSON key files to be found",

			ARG arg_wallet_path: (String) = "$BASE/wallet", or |c: &Config| c.parity.as_ref()?.wallet_path.clone(),
			"--wallet-path=[PATH]",
			"Specify the path for wallet files to be found",
		...
		["Convenience Options"]
			...
			"--ports-shift=[SHIFT]",
			"Add SHIFT to all port numbers Origo is listening on. Includes network port and all servers (HTTP JSON-RPC, WebSockets JSON-RPC, IPFS, SecretStore).",
		...
		["Networking Options"]
			...
			ARG arg_port: (u16) = 26622u16, or |c: &Config| c.network.as_ref()?.port.clone(),
			"--port=[PORT]",
			...
			"--interface=[IP]",
			"Network interfaces. Valid values are 'all', 'local' or the ip of the interface you want origo to listen to.",
		...
		["API and Console Options – HTTP JSON-RPC"]
			...
			ARG arg_jsonrpc_port: (u16) = 6622u16, or |c: &Config| c.rpc.as_ref()?.port.clone(),
			"--jsonrpc-port=[PORT]",
			...
			ARG arg_jsonrpc_apis: (String) = "web3,eth,origo,pubsub,net,parity,parity_pubsub,traces,rpc,shh,shh_pubsub", or |c: &Config| c.rpc.as_ref()?.apis.as_ref().map(|vec| vec.join(",")),
			"--jsonrpc-apis=[APIS]",
			"Specify the APIs available through the HTTP JSON-RPC interface using a comma-delimited list of API names. Possible names are: all, safe, debug, web3, net, eth, pubsub, personal, signer, origo, traces, rpc, secretstore, shh, shh_pubsub. You can also disable a specific API by putting '-' in the front, example: all,-personal. 'safe' enables the following APIs: web3, net, eth, pubsub, parity, parity_pubsub, traces, rpc, shh, shh_pubsub",
		...
		["API and Console Options – WebSockets"]
			...
			ARG arg_ws_port: (u16) = 6623u16, or |c: &Config| c.websockets.as_ref()?.port.clone(),
			"--ws-port=[PORT]",
			...
			ARG arg_ws_apis: (String) = "web3,eth,origo,pubsub,net,parity,parity_pubsub,traces,rpc,shh,shh_pubsub", or |c: &Config| c.websockets.as_ref()?.apis.as_ref().map(|vec| vec.join(",")),
			"--ws-apis=[APIS]",
			"Specify the JSON-RPC APIs available through the WebSockets interface using a comma-delimited list of API names. Possible names are: all, safe, web3, net, eth, pubsub, personal, signer, traces, rpc, secretstore, shh, shh_pubsub. You can also disable a specific API by putting '-' in the front, example: all,-personal. 'safe' enables the following APIs: web3, net, eth, pubsub, parity, parity_pubsub, traces, rpc, shh, shh_pubsub",
		...
		["API and Console Options – IPC"]
			...
			ARG arg_ipc_apis: (String) = "web3,eth,origo,pubsub,net,parity,parity_pubsub,parity_accounts,traces,rpc,shh,shh_pubsub", or |c: &Config| c.ipc.as_ref()?.apis.as_ref().map(|vec| vec.join(",")),
			"--ipc-apis=[APIS]",
			"Specify custom API set available via JSON-RPC over IPC using a comma-delimited list of API names. Possible names are: all, safe, web3, net, eth, pubsub, personal, signer, traces, rpc, secretstore, shh, shh_pubsub. You can also disable a specific API by putting '-' in the front, example: all,-personal. 'safe' enables the following APIs: web3, net, eth, pubsub, parity, parity_pubsub, traces, rpc, shh, shh_pubsub",
		...
		["Sealing/Mining Options"]
			...
			ARG arg_stratum_port: (u16) = 6002u16, or |c: &Config| c.stratum.as_ref()?.port.clone(),
			"--stratum-port=[PORT]",
...
mod tests {
	...
	fn should_disable_stratum() {
		...
		assert_eq!(args.arg_stratum_port, 6002u16);
	...
	fn should_parse_full_config() {
		...
		assert_eq!(args, Args {
			...
			arg_chain: "xyz".into(),
			arg_base_path: Some("$HOME/.origo".into()),
			arg_db_path: Some("$HOME/.origo/chains".into()),
			arg_keys_path: "$HOME/.origo/keys".into(),
			arg_wallet_path: "$HOME/.origo/wallet".into(),
			arg_identity: "".into(),
			flag_light: false,
			...
			arg_ui_hosts: None,
			arg_ui_path: "$HOME/.origo/signer".into(),
			...
			arg_secretstore_path: "$HOME/.origo/secretstore".into(),
			...
			arg_extra_data: Some("Origo".into()),
			...
			arg_log_file: Some("/var/log/origo.log".into()),
			flag_no_color: false,
	...
	fn should_deserialize_toml_file() {
		let config: Config = toml::from_str(include_str!("./tests/config.base.toml")).unwrap();
		...
		assert_eq!(config, Config {
			...
			misc: Some(Misc {
				...
				log_file: Some("/var/log/origo.log".into()),
				color: Some(true),
				ports_shift: Some(0),
