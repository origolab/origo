...
impl Configuration {
	...
	pub(crate) fn into_command(self) -> Result<Execute, String> {
		...
		/// TODO(Kui): Temporary disable the warp sync because insert_unordered_block can not
		/// work properly for commitment tree storage.
	...
	fn gas_pricer_config(&self) -> Result<GasPricerConfig, String> {
		...
		} else if self.chain()? == SpecType::Medietas || self.chain()? == SpecType::Medietatis {
			return Ok(GasPricerConfig::Fixed(U256::from(1_000_000_000)));
	...
	fn directories(&self) -> Directories {
		...
		let wallet_path = replace_home(&data_path, &self.args.arg_wallet_path);
...
mod tests {
	...
	fn should_ignore_comments_in_reserved_peers() {
		...
		File::create(&filename).unwrap().write_all(b"# Sample comment\nenode://6f8a80d14311c39f35f516fa664deaaaa13e85b2f7493f37f6144d86991ec012937307647bd3b9a82abe2974e1407241d54947bbb39763a4cac9f77166ad92a0@172.0.0.1:26622\n").unwrap();
		let args = vec!["parity", "--reserved-peers", filename.to_str().unwrap()];
		let conf = Configuration::parse_cli(&args).unwrap();
	...
	fn test_identity_arg() {
		...
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				...
				assert!(c.net_conf.client_version.starts_with("Origo/Somebody/"));
			}
			_ => panic!("Should be Cmd::Run"),
		}
	}
	...
	fn should_apply_ports_shift() {
		...
		let conf1 = parse(&["parity", "--ports-shift", "1", "--jsonrpc-port", "6622"]);

		// then
		assert_eq!(conf0.net_addresses().unwrap().0.port(), 26623);
		assert_eq!(conf0.network_settings().unwrap().network_port, 26623);
		assert_eq!(conf0.network_settings().unwrap().rpc_port, 6623);
		assert_eq!(conf0.http_config().unwrap().port, 6623);
		assert_eq!(conf0.ws_config().unwrap().port, 6624);
		...
		assert_eq!(conf0.stratum_options().unwrap().unwrap().port, 6003);

		assert_eq!(conf1.net_addresses().unwrap().0.port(), 26623);
		assert_eq!(conf1.network_settings().unwrap().network_port, 26623);
		assert_eq!(conf1.network_settings().unwrap().rpc_port, 6623);
		assert_eq!(conf1.http_config().unwrap().port, 6623);
		assert_eq!(conf1.ws_config().unwrap().port, 6624);
