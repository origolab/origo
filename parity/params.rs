...
impl str::FromStr for SpecType {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let spec = match s {
			...
			"medietas" => SpecType::Medietas,
			"medietas-testnet" => SpecType::Medietatis,
			"dev" => SpecType::Dev,
...
impl fmt::Display for SpecType {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		f.write_str(match *self {
			...
			SpecType::Medietas => "medietas",
			SpecType::Medietatis => "medietatis",
			SpecType::Dev => "dev",
...
impl SpecType {
	pub fn spec<'a, T: Into<SpecParams<'a>>>(&self, params: T) -> Result<Spec, String> {
		...
		match *self {
			...
			SpecType::Medietas => Ok(ethereum::new_medietas(params)),
			SpecType::Medietatis => Ok(ethereum::new_medietatis(params)),
...
impl Default for GasPricerConfig {
	fn default() -> Self {
		...
		GasPricerConfig::Fixed(U256::from(1_000_000_000))
	}
}
...
mod tests {
	...
	fn test_spec_type_parsing() {
		...
		assert_eq!(SpecType::Medietas, "medietas".parse().unwrap());
	...
	fn test_spec_type_default() {
		assert_eq!(SpecType::Medietas, SpecType::default());
	}
	...
	fn test_spec_type_display() {
		...
		assert_eq!(format!("{}", SpecType::Medietas), "medietas");
