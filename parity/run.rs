	...
	where Cr: Fn(String) + 'static + Send
{
	...
	// load spec
	let spec = cmd.spec.spec(SpecParams::new(cmd.dirs.cache.as_ref()))?;
		...
		Rr: Fn() + 'static + Send
{
	...
	//save reward configure in client config
	client_config.blockchain.reward_config = spec.params().reward_config.clone();
