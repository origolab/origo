...
use types::transaction::{SignedTransaction, PendingTransaction, Error as TransactionError, PrivateTransaction};
	...
	OD: OnDemandRequester + 'static
{
		...
		-> BoxFuture<FilledTransactionRequest>
	{
		...
		let private = match request.shield_amounts {
			// TODO(xin): use the AmountRequest in request to create the private.
			Some(_) => Some(PrivateTransaction::create_private(vec![0])),
			None => None,
		};
