	...
	EM: ExternalMinerService + 'static,
{
	...
	fn submit_work(&self, nonce: H256, pow_hash: H512, solution: Bytes) -> Result<bool> {
		match helpers::submit_work_detail(&self.client, &self.miner, nonce, pow_hash, solution) {
			Ok(_)  => Ok(true),
			Err(_) => Ok(false),
		}
	}
