...
use ethcore::client::{Nonce, TransactionInfo, PrepareOpenBlock, StateClient, EngineInfo};
...
impl MinerService for TestMinerService {
	...
	fn import_external_transactions<C: Nonce + TransactionInfo + Sync>(&self, chain: &C, transactions: Vec<UnverifiedTransaction>)
	...
	fn import_claimed_local_transaction<C: Nonce + TransactionInfo + Sync>(&self, chain: &C, pending: PendingTransaction, _trusted: bool)
		-> Result<(), transaction::Error> {
	...
	fn work_package<C: PrepareOpenBlock>(&self, chain: &C) -> Option<(H512, BlockNumber, u64, U256)> {
		...
		Some((header.pow_hash(), header.number(), header.timestamp(), *header.difficulty()))
	}
	...
	fn submit_seal(&self, _pow_hash: H512, _seal: Vec<Bytes>) -> Result<SealedBlock, Error> {
		unimplemented!();
	}
