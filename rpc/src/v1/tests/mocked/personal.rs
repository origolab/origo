...
#[test]
fn send_shield_transaction_with_invalid_amount() {
	let tester = setup();
	let address = tester.accounts.new_account(&"password123".into()).unwrap();
	let (action, private, request) = (
		Action::PublicToPrivate,
		Some(PrivateTransaction::create_private(vec![0])),
		r#"{
		"jsonrpc": "2.0",
		"method": ""#.to_owned() + "personal_sendShieldTransaction" + r#"",
		"params": [{
		"from": ""# + format!("0x{:x}", address).as_ref() + r#"",
		"to": "0xd46e8dd67c5d32be8058bb8eb970870f07244567",
		"gas": "0x76c0",
		"gasPrice": "0x9184e72a000",
		"value": "0x2000000000",
		"shieldAmounts": [{"address":"ogo175j7xj6jgn3w0trmxzmssydmdq5rd9vxydwdqmd9t6qkykrr0y24w6xfp44knukqjweuxxa9mel",
							"amount": "0x2000000000",
							"memo":"test"
						}]
		}, "password123"],
	...
	let t = Transaction {
		...
		action: action.clone(),
		value: U256::from(0x2000000000u64),
		data: vec![],
	...
	let res = tester.io.handle_request_sync(request.as_ref());
	assert_eq!(res.unwrap(), String::from("{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32081,\"message\":\"Transaction builder failed.The balance should be the multiple of 10^9.\"},\"id\":1}") );
}
...
#[test]
fn send_shield_transaction() {
	sign_and_send_test("personal_sendShieldTransaction");
}
...
fn sign_and_send_test(method: &str) {
	let tester = setup();
	let address = tester.accounts.new_account(&"password123".into()).unwrap();
	let (action, private, request) = match method.contains("Shield") {
		true => (Action::PublicToPrivate,
				 Some(PrivateTransaction::create_private(vec![0])),
				 r#"{
				 "jsonrpc": "2.0",
				 "method": ""#.to_owned() + method + r#"",
		...
		false => (Action::Call(Address::from_str("d46e8dd67c5d32be8058bb8eb970870f07244567").unwrap()),
				  None,
				  r#"{
				  "jsonrpc": "2.0",
				  "method": ""#.to_owned() + method + r#"",
