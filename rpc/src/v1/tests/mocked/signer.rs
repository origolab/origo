...
use rlp::encode;
use types::transaction::{Transaction, Action, SignedTransaction, PrivateTransaction};
...
fn should_return_list_of_items_to_confirm() {
	...
	let response = concat!(
		...
		r#"{"id":"0x1","origin":"unknown","payload":{"sendTransaction":{"condition":null,"data":"0x","from":"0x0000000000000000000000000000000000000001","gas":"0x989680","gasPrice":"0x2710","nonce":null,"shieldAmounts":null,"to":"0xd46e8dd67c5d32be8058bb8eb970870f07244567","value":"0x1"}}},"#,
...
fn should_confirm_private_isnt_null_sign_transaction_with_rlp() {
		// given
	let tester = signer_tester();
	let address = tester.accounts.new_account(&"test".into()).unwrap();
	let recipient = Address::from_str("d46e8dd67c5d32be8058bb8eb970870f07244567").unwrap();
	...
	let t = Transaction {
		...
		value: U256::from(0x1),
		data: vec![],
		private: Some(PrivateTransaction::create_private(vec![0])),
	};
	let signature = tester.accounts.sign(address, Some("test".into()), t.hash(None)).unwrap();
	let t = SignedTransaction::new(t.with_signature(signature.clone(), None)).unwrap();
	let rlp = encode(&t);
