...
pub struct Transaction {
	...
	/// Flag of private transaction.
	pub is_private: bool,
	/// Balancing value.
	pub balancing_value: Option<i64>,
}
	...
	pub fn from_localized(mut t: LocalizedTransaction) -> Transaction {
		...
		Transaction {
			...
			to: match t.action {
				...
				Action::PrivateCall(ref address) => Some(*address)
			},
			value: t.value,
			...
			creates: match t.action {
				...
				Action::Call(_) | Action::PrivateToPrivate |
				Action::PublicToPrivate | Action::PrivateCall(_) => None,
			},
			...
			s: signature.s().into(),
			condition: None,
			is_private: t.private.is_some(),
			balancing_value: match t.private {
				None => None,
				Some(_) => Some(t.private.clone().unwrap().balancing_value),
			},
		}
	}
