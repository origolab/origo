...
impl From<helpers::TransactionRequest> for TransactionRequest {
	fn from(r: helpers::TransactionRequest) -> Self {
		TransactionRequest {
			...
			shield_amounts: match r.shield_amounts {
				None => None,
				Some(requests)=> Some(requests.into_iter().map(|x| AmountRequest::from(x)).collect()),
			}
		}
	}
}
...
impl Into<helpers::TransactionRequest> for TransactionRequest {
	fn into(self) -> helpers::TransactionRequest {
		helpers::TransactionRequest {
			...
			shield_amounts: match self.shield_amounts {
				None => None,
				Some(requests)=> Some(requests.into_iter().map(|x| helpers::AmountRequest::from(x)).collect()),
			}
		}
	}
}
