...
impl Default for Directories {
	fn default() -> Self {
		...
		Directories {
			...
			wallet: replace_home(&data_dir, "$BASE/wallet"),
		}
	}
}
...
impl Directories {
	...
	pub fn create_dirs(&self, signer_enabled: bool, secretstore_enabled: bool) -> Result<(), String> {
		...
		fs::create_dir_all(&self.wallet).map_err(|e| e.to_string())?;
		if signer_enabled {
...
pub fn default_data_path() -> String {
	...
	get_app_root(AppDataType::UserData, &app_info).map(|p| p.to_string_lossy().into_owned()).unwrap_or_else(|_| "$HOME/.origo".to_owned())
}

/// Default local path
pub fn default_local_path() -> String {
	let app_info = AppInfo { name: PRODUCT, author: AUTHOR };
	get_app_root(AppDataType::UserCache, &app_info).map(|p| p.to_string_lossy().into_owned()).unwrap_or_else(|_| "$HOME/.origo".to_owned())
}

/// Default hypervisor path
pub fn default_hypervisor_path() -> PathBuf {
	let app_info = AppInfo { name: PRODUCT_HYPERVISOR, author: AUTHOR };
	get_app_root(AppDataType::UserData, &app_info).unwrap_or_else(|_| "$HOME/.origo-hypervisor".into())
}
...
mod platform {
	...
	pub const AUTHOR: &str = "Origo";
	pub const PRODUCT: &str = "network.origo";
	pub const PRODUCT_HYPERVISOR: &str = "network.origo.updates";

	pub fn parity_base() -> PathBuf {
		let mut home = super::home();
		home.push("Library");
		home.push("Application Support");
		home.push("network.origo");
		home.push("keys");
		home
	}
